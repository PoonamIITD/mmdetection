...
class HybridDINO(DINO):
    ...
    def __init__(self,
        ...
        self.dino_backbone_cfg = dino_backbone
        self.dino_neck_cfg = dino_neck
    ...
    def _init_layers(self) -> None:
        ...
        # self.dino_positional_encoding = SinePositionalEncoding(**self.dino_positional_encoding)
        ...
        self.dino_level_embed = nn.Parameter(
            torch.Tensor(self.num_feature_levels, self.embed_dims))
        ...
        self.dino_backbone = MODELS.build(self.dino_backbone_cfg)
        self.dino_neck = MODELS.build(self.dino_neck_cfg)
        ...
        for param in self.dino_neck.parameters():
            param.requires_grad = False
        ...
        self.dino_level_embed.requires_grad = False
        ...
        for param in self.bbox_head.parameters():
         ...
         # Freeze encoder auxiliary branch only
        enc_cls_branch = self.bbox_head.cls_branches[self.decoder.num_layers]
        enc_reg_branch = self.bbox_head.reg_branches[self.decoder.num_layers]
    ...
    def init_weights(self):
        ...
        # -------- DINO: backbone --------
        if hasattr(self, "dino_backbone"):
            backbone_weights = {
                k.replace("backbone.", ""): v
                for k, v in dino_ckpt.items() if k.startswith("backbone.")
            }

            missing, unexpected = self.dino_backbone.load_state_dict(backbone_weights, strict=False)
            # print(self.backbone.state_dict().keys())
            # print([k for k in gdino_ckpt.keys() if k.startswith("backbone.")])
        ...
        # -------- DINO: neck --------
        if hasattr(self, "dino_neck"):
            neck_weights = {
                k.replace("neck.", ""): v
                for k, v in dino_ckpt.items() if k.startswith("neck.")
            }
            missing, unexpected = self.dino_neck.load_state_dict(neck_weights, strict=True)
        ...
        # -------- DINO: level_embed --------
        if "dino_level_embed" in dino_ckpt:
            with torch.no_grad():
                self.dino_level_embed.copy_(dino_ckpt["level_embed"])
    ...
    def forward_transformer(
        ...
        with torch.no_grad():
            ...
            dino_encoder_inputs_dict, _ = self.pre_transformer(
                dino_img_feats, batch_data_samples=batch_data_samples, pre='dino')

            dino_feat = dino_encoder_inputs_dict['feat'] 
            encoder_outputs_dict = self.forward_encoder(
                **encoder_inputs_dict, dino_feat=dino_feat , text_dict=text_dict)
    ...
    def pre_transformer(
        ...
        for lvl, (feat, mask, pos_embed) in enumerate(
            ...
            with torch.no_grad():
                if pre=='dino':
                    lvl_pos_embed = pos_embed + self.dino_level_embed[lvl].view(1, 1, -1)
                else:    
    ...
    def extract_dino_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        """Extract features.
        ...
        """
        x = self.dino_backbone(batch_inputs)
        x = self.dino_neck(x)
        return x
        ...
        with torch.no_grad():
            if self.use_autocast:
                with autocast(enabled=True):
                    ...
                    dino_visual_features = self.extract_dino_feat(batch_inputs)

            else:
                visual_features = self.extract_feat(batch_inputs)
                dino_visual_features = self.extract_dino_feat(batch_inputs)
        head_inputs_dict = self.forward_transformer(visual_features, dino_visual_features, text_dict,
