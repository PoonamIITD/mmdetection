...
class GroundingDINOHead(DINOHead):
    ...
    def compute_query_level_alignment(self,
                                      hidden_state,
                                      descriptor_embedded,
                                      descriptor_mask,
                                      class_embeds,
        ...
        descriptor_mask: [bs, total_tokens]
        class_embeds: [K, C]
        descriptor_ranges_per_class: list of (start, end)
        """

        bs, Q, C = hidden_state.shape
        K = len(descriptor_ranges_per_class)
        ...
        for k in range(K):
            ...
            # FIX: descriptor_ranges_per_class[k] is a LIST of (start,end)
            ranges = descriptor_ranges_per_class[k]

            # ---- gather all descriptor tokens for class k ----
            desc_embeds_per_descriptor = []
            mask_per_descriptor = []

            for (start, end) in ranges:
                tokens = descriptor_embedded[:, start:end, :]   # [bs, T, C]
                mask   = descriptor_mask[:, start:end]          # [bs, T]
                ...
                desc_embed = summed / count                     # [bs, C]

                desc_embeds_per_descriptor.append(desc_embed)
            ...
            v_j = torch.softmax(sim_text, dim=-1) # [bs, 4] CORRECT

            # ---- compute similarity between queries and descriptors ----
            sim_cross = torch.matmul(hidden_state, desc_embeds.transpose(1, 2))  # [bs,Q,4]

            # ---- weighted sum: Σ_j v_j * Sim(q, y_j) ----
            score_qk = (v_j.unsqueeze(1) * sim_cross).sum(dim=-1)  # [bs,Q]

            all_scores.append(score_qk)

        # ---- stack across classes ----
        desc_scores = torch.stack(all_scores, dim=-1)  # [bs,Q,K]
    ...
    def forward(
        ...
        for layer_id in range(hidden_states.shape[0]):
            ...
            if (descriptor_embedded is not None) and (layer_id == hidden_states.shape[0] - 1):

                desc_scores = self.compute_query_level_alignment(
                    hidden_state,
                    descriptor_embedded,
                    descriptor_mask,
                    class_embeds,
                ...
                desc_scores_last_layer = desc_scores / math.sqrt(hidden_state.shape[-1])
        ...
        return all_layers_outputs_classes, all_layers_outputs_coords, desc_scores_last_layer
    ...
    def predict(self,
                ...
                rescale: bool = True,
                descriptor_embedded: Optional[Tensor] = None,
                descriptor_mask: Optional[Tensor] = None,
                class_embeds: Optional[Tensor] = None,
                descriptor_ranges_per_class: Optional[list] = None,
                ) -> InstanceList:
    ...
    def predict_by_feat(self,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            desc_scores = desc_scores_last_layer [img_id]
            token_positive_maps = batch_token_positive_maps[img_id]
            results = self._predict_by_feat_single(cls_score, bbox_pred,
                                                   token_positive_maps,
                                                   img_meta, rescale, desc_scores)
    ...
    def _predict_by_feat_single(self,
                                ...
                                img_meta: dict,
                                rescale: bool = True,
                                desc_scores: Tensor = None) -> InstanceData:
        ...
        desc_labels = desc_scores.argmax(dim=-1)        # [N]
        desc_conf   = desc_scores.max(dim=-1).values 
        ...
        results.desc_scores = desc_scores
        results.desc_labels = desc_labels
        results.desc_conf = desc_conf
        return results
