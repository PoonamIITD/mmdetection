...
class PromptedShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        # remove the appended prompts before window partitioning
        ...
        # add back the prompt to each window for attention computation
...
class PromptedSwinBlockSequence(BaseModule):
    ...
    def __init__(self,
        ...
        self.num_prompts = prompt_tokens
        self.prompt_location = prompt_location
    ...
    def forward(self, x, hw_shape, deep_prompt_embd=None):
        ...
        if not self.prompt_deep:
            ...
            # add the prompt embedding before each block call
            B = x.shape[0]  # batchsize
            num_blocks = len(self.blocks)
            if deep_prompt_embd.shape[0] != num_blocks:
                # first layer (stage)
                for i in range(num_blocks):
                    # do not apply prompt to first block of first layer
                    if i == 0:
        ...
        # downsample at the end of the stage (after all blocks)
        if self.downsample:
            x_down, down_hw_shape = self.downsample(x, hw_shape)
            # the downsampled features go to the next stage - keep prompt tokens there
            # the original-sized features are reshaped to form the outputs - remove prompts tokens from this
            if self.prompt_location == "prepend":
...
class PromptedSwinTransformer(BaseModule):
    ...
    def train(self, mode=True):
        ...
        # # after setting the model to train mode, freeze certain layer as specified by self.frozen_stages
        # self._freeze_stages()
    ...
    def forward(self, x):
        ...
        # compute image patch embeddings - (batch_size, n_patches, hidden_dim)
        x, hw_shape = self.patch_embed(x)
        if self.use_abs_pos_embed:
            x = x + self.absolute_pos_embed
        x = self.drop_after_pos(x)

        # compute prompt embeddings - - (batch_size, n_prompt, hidden_dim)
        B = x.shape[0]
        prompt_embd = self.prompt_dropout(
            self.prompt_embeddings.expand(B, -1, -1))

        # concatenate prompt embeddings: after CLS token, all before image patches
        ...
        outs = []
        if self.prompt_deep is True:
            for i, (stage, deep_prompt_embd) in enumerate(zip(
                self.stages, [self.deep_prompt_embeddings_0,
            ...
            for i, stage in enumerate(self.stages):
                # out is the output of the layer ~ prior to downsamling
                # x is the downsampled output that serves as input to the next layer
