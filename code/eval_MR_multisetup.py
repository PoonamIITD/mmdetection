...
class COCOeval:
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, maxDet):
        ...
        dtind_sorted = np.argsort([-d['score'] for d in dt], kind='mergesort')
        dt_sorted = [dt[i] for i in dtind_sorted[0:maxDet]]

        # exclude dt out of height range, but KEEP the original position of each
        # surviving detection in dt_sorted so we index the ious matrix correctly
        keep_positions = []
        dt = []
        for pos, d in enumerate(dt_sorted):
            if d['height'] >= hRng[0] / self.params.expFilter and d['height'] < hRng[1] * self.params.expFilter:
                dt.append(d)
                keep_positions.append(pos)
        dtind = np.array(keep_positions, dtype=int)
