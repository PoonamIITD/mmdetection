...
def print_summary_table(all_results, subset_names):
    ...
    for subset_name in subset_names:
        ...
        rows.append([subset_name,
                     ...
                     f"{n_pos:,}", f"{n_img:,}"])
    print(f"\n{'='*100}")
    print("  CityPersons MR⁻² (REAL official code: coco.py + eval_MR_multisetup.py)  |  lower is better")
    print(f"{'='*100}")
    if HAS_TABULATE:
        print(tabulate(rows, headers=header, tablefmt="rounded_outline"))
    else:
...
def main():
    ...
    for subset_name, subset_def in subset_defs.items():
        ...
        print(f"{'─'*60}")
        all_results[subset_name] = {}
        for model_name, preds_official in [("baseline", b_preds_official), ("updated", u_preds_official)]:
